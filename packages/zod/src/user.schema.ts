import { providers, type users } from "@posts-app/database/schema";
import { z } from "zod";
...
export const UserDatabaseWithoutPasswordSchema = UserDatabaseSchema.omit({
	password: true,
});
...
export const UserSettingsSchema = UserSchema.pick({
	...
	profilePicture: z
		...
		.refine(
			...
			"Media file must be less than 5 MB",
		)
		.or(z.null())
		.or(z.undefined()),
});
